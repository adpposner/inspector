%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function SP2_MRSI_PlotSpecDiff( f_new )
%%
...
if flag.mrsiFormat==1           % real part
    [minI,maxI,ppmZoom,specZoom,f_done] = SP2_MRSI_ExtractPpmRange(ppmMin,ppmMax,mrsi.ppmCalib,...
                                                               mrsi.spec1.sw,real(mrsi.specDiff));
elseif flag.mrsiFormat==2       % imaginary part
    [minI,maxI,ppmZoom,specZoom,f_done] = SP2_MRSI_ExtractPpmRange(ppmMin,ppmMax,mrsi.ppmCalib,...
                                                               mrsi.spec1.sw,imag(mrsi.specDiff));
elseif flag.mrsiFormat==3       % magnitude
    [minI,maxI,ppmZoom,specZoom,f_done] = SP2_MRSI_ExtractPpmRange(ppmMin,ppmMax,mrsi.ppmCalib,...
                                                               mrsi.spec1.sw,abs(mrsi.specDiff));
else                            % phase
    [minI,maxI,ppmZoom,specZoom,f_done] = SP2_MRSI_ExtractPpmRange(ppmMin,ppmMax,mrsi.ppmCalib,...
                                                               mrsi.spec1.sw,angle(mrsi.specDiff));
end                                             
if ~f_done
...
if f_new && isfield(mrsi,'fhSpecDiff') && ~flag.mrsiKeepFig
    if ishandle(mrsi.fhSpecDiff)
        delete(mrsi.fhSpecDiff)
    end
    mrsi = rmfield(mrsi,'fhSpecDiff');
end
% create figure if necessary
if ~isfield(mrsi,'fhSpecDiff') || ~ishandle(mrsi.fhSpecDiff)
    mrsi.fhSpecDiff = figure('IntegerHandle','off');
    set(mrsi.fhSpecDiff,'NumberTitle','off','Name',sprintf(' Difference Spectrum'),...
        'Position',[650 45 560 550],'Color',[1 1 1]);
else
    set(0,'CurrentFigure',mrsi.fhSpecDiff)
    if flag.mrsiKeepFig
        if ~SP2_KeepFigure(mrsi.fhSpecDiff)
            return
        end
    end
end
...
if flag.mrsiAnaSNR || flag.mrsiAnaFWHM || flag.mrsiAnaIntegr
    ...
    if ~SP2_MRSI_Analysis(mrsi.specDiff,mrsi.spec1,plotLim)
        return
    end
end

%--- export handling ---
mrsi.expt.fid    = ifft(ifftshift(mrsi.specDiff,1),[],1);
