function [pulseStruct] = readFile(filename)
load(filename)
...
if (~autoMean)
    disp('No automatic summing, be careful as this may take a long time for large number of spatial points and/or may run out of memory')
end
...
%fill out the metabolite cell
if (~exist('metabolites','var'))
    error('Error, no metabolites found in input .mat file!');
else
    pulseStruct.metabolites = metabolites;
end
...
if (exist('B0','var'))
    ...
    error('Must input field strength in Tesla')
end
