...
FCTNAME = 'SP2_Proc_Spec1DataRawUpdate';
...
if ~strcmp(dat1FidFileTmp(end-3:end),'.raw') && ...
   ~strcmp(dat1FidFileTmp(end-3:end),'.RAW') && ...
   ~strcmp(dat1FidFileTmp(end-3:end),'.h2o') && ...
   ~strcmp(dat1FidFileTmp(end-3:end),'.H2O')
...
proc.spec1.dataPathRaw   = get(fm.proc.spec1DataPath,'String');
proc.spec1.dataPathTxt   = [proc.spec1.dataPathRaw(1:end-4) '.txt'];
proc.spec1.dataPathPar   = [proc.spec1.dataPathRaw(1:end-4) '.par'];
proc.spec1.dataPathMat   = [proc.spec1.dataPathRaw(1:end-4) '.mat'];
proc.spec1.dataPathCoord = [proc.spec1.dataPathRaw(1:end-4) '.coord'];

%--- update paths ---
if flag.OS>0                % 1: linux, 2: mac
    slInd = find(proc.spec1.dataPathRaw=='/');
else                        % 0: PC
    slInd = find(proc.spec1.dataPathRaw=='\');
end
proc.spec1.dataDir       = proc.spec1.dataPathRaw(1:slInd(end));
