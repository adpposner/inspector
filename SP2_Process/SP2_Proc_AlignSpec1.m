%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_succ = SP2_Proc_AlignSpec1
%%
...
flag.procSpec1Lb      = flag.procSpec1Lb || flag.procAlignLb;
flag.procSpec1Gb      = flag.procSpec1Gb || flag.procAlignGb;
flag.procSpec1Phc0    = flag.procSpec1Phc0 || flag.procAlignPhc0;
flag.procSpec1Phc1    = flag.procSpec1Phc1 || flag.procAlignPhc1;
flag.procSpec1Scale   = flag.procSpec1Scale || flag.procAlignScale;
flag.procSpec1Shift   = flag.procSpec1Shift || flag.procAlignShift;
flag.procSpec1Offset  = flag.procSpec1Offset || flag.procAlignOffset;
flag.procSpec1Stretch = flag.procSpec1Stretch || flag.procAlignStretch;
set(fm.proc.spec1LbFlag,'Value',flag.procSpec1Lb==1);
set(fm.proc.spec1GbFlag,'Value',flag.procSpec1Gb==1);
set(fm.proc.spec1Phc0Flag,'Value',flag.procSpec1Phc0==1);
set(fm.proc.spec1Phc1Flag,'Value',flag.procSpec1Phc1==1);
set(fm.proc.spec1ScaleFlag,'Value',flag.procSpec1Scale==1);
set(fm.proc.spec1ShiftFlag,'Value',flag.procSpec1Shift==1);
set(fm.proc.spec1OffsetFlag,'Value',flag.procSpec1Offset==1);
set(fm.proc.spec1StretchFlag,'Value',flag.procSpec1Stretch==1);
SP2_Proc_ProcessWinUpdate

%--- extraction of spectral parts to be fitted ---
alignAllBin = zeros(1,proc.spec2.nspecC);        % init global loggingfile index vector for ppm ranges to be used (binary format)
...
for winCnt = 1:proc.alignPpmN
    [alignMinI(winCnt),alignMaxI(winCnt),ppmZoom,specZoom,f_done] = ...
        SP2_Proc_ExtractPpmRange(proc.alignPpmMin(winCnt),proc.alignPpmMax(winCnt),...
                                 proc.ppmCalib,proc.spec2.sw,proc.spec2.spec);
    if ~f_done
...
if flag.procAlignPoly
    proc.ppmVec         = (-proc.spec1.sw/2:proc.spec1.sw/(proc.spec1.nspecC-1):proc.spec1.sw/2)' + proc.ppmCalib;
...
refSpecZoom = proc.spec2.spec(proc.alignAllInd);
...
if flag.procAlignLb
    ...
    % parameter handling
    lb = [lb -5];
    coeffStart = [coeffStart proc.spec1.lb];
    ub = [ub 20];
end
if flag.procAlignGb
    % index handling
    ...
    % parameter handling
    lb = [lb -5];
    coeffStart = [coeffStart proc.spec1.gb];
    ub = [ub 20];
end
if flag.procAlignShift
    ...
    % parameter handling
    lb = [lb proc.spec1.shift-20];
    coeffStart = [coeffStart proc.spec1.shift];
    ub = [ub proc.spec1.shift+20];
end
if flag.procAlignOffset
    ...
    % parameter handling
    lb = [lb proc.spec1.offset-20];
    coeffStart = [coeffStart proc.spec1.offset];
    ub = [ub proc.spec1.offset+20];
end
if flag.procAlignPhc0
    ...
    % parameter handling
    lb = [lb -359.999];
    coeffStart = [coeffStart proc.spec1.phc0];
    ub = [ub 360];
end
if flag.procAlignPhc1
    ...
    % parameter handling
    lb = [lb -1000];
    coeffStart = [coeffStart proc.spec1.phc1];
    ub = [ub 1000];
end
if flag.procAlignScale
    ...
    % parameter handling
    lb = [lb 0];
    coeffStart = [coeffStart proc.spec1.scale];
    ub = [ub 30];
end
if flag.procAlignStretch
    ...
    % parameter handling
    lb = [lb -proc.spec1.sf];
    coeffStart = [coeffStart proc.spec1.stretch];
    ub = [ub proc.spec1.sf];
end
if flag.procAlignPoly
    % index handling
    ...
    % parameter handling
%     lb = [lb -1e5*ones(1,proc.alignPolyOrder+1)];
%     coeffStart = [coeffStart zeros(1,proc.alignPolyOrder+1)];
%     ub = [ub 1e5*ones(1,proc.alignPolyOrder+1)];
    lb = [lb -1e4*ones(1,proc.alignPolyOrder+1)];
    coeffStart = [coeffStart proc.spec1.polycoeff(end-proc.alignPolyOrder:end)];     % (reversed) nth..0 order
...
align.frequVecOrig = -proc.spec1.sw_h/2:proc.spec1.sw_h/(proc.spec1.nspecC-1):proc.spec1.sw_h/2;
align.frequVecNew  = align.frequVecOrig;        % init only
align.specReal     = real(proc.spec1.spec);     % init only
align.specImag     = imag(proc.spec1.spec);     % init only
...
[coeffFit,resnorm,resid,exitflag,output] = ...
    lsqcurvefit('SP2_Proc_AlignSpec1FitFct',coeffStart,[],refSpecZoomFit,lb,ub,opt);

%--- parameter assignment ---
if flag.procAlignLb
    proc.spec1.lb     = coeffFit(align.indLb);
end
if flag.procAlignGb
    proc.spec1.gb     = coeffFit(align.indGb);
end
if flag.procAlignPhc0
    proc.spec1.phc0   = coeffFit(align.indPhc0);
end
if flag.procAlignPhc1
    proc.spec1.phc1   = coeffFit(align.indPhc1);
end
if flag.procAlignScale
    proc.spec1.scale  = coeffFit(align.indScale);
end
if flag.procAlignShift
    proc.spec1.shift  = coeffFit(align.indShift);
end
if flag.procAlignOffset
    proc.spec1.offset = coeffFit(align.indOffset);
end
if flag.procAlignStretch
    proc.spec1.stretch = coeffFit(align.indStretch);
end
if flag.procAlignPoly
    proc.spec1.polycoeff(end-proc.alignPolyOrder:end) = coeffFit(align.indPoly:end); 
end

%--- info printout ---
fprintf('\nAlignment result of spectrum 1:\n');
if flag.procAlignLb
    fprintf('LB:\t\t\t%.3f Hz\n',proc.spec1.lb);
end
if flag.procAlignGb
...
if flag.procAlignPoly
    ...
    fprintf('Poly (complete):\t%s\n',SP2_Vec2PrintStr(proc.spec1.polycoeff,2));
end
fprintf('Resnorm:\t%f\n\n',resnorm);
...
if flag.procAlignOffset
    set(fm.proc.spec1OffsetVal,'String',sprintf('%.4f',proc.spec1.offset));
end
if flag.procAlignStretch
    set(fm.proc.spec1StretchVal,'String',sprintf('%.5f',proc.spec1.stretch));
end
...
if flag.procAlignPoly
    ...
    switch proc.alignPolyOrder
        case 0
            polyFit = coeffFit(align.indPoly)*ones(1,proc.spec1.nspecC)';
        case 1
            polyFit = coeffFit(align.indPoly)*proc.ppmVec + ...
                      coeffFit(align.indPoly+1);
        case 2
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^2 + ...
                      coeffFit(align.indPoly+1)*proc.ppmVec + ...
                      coeffFit(align.indPoly+2);
        case 3
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^3 + ...
                      coeffFit(align.indPoly+1)*proc.ppmVec.^2 + ...
                      coeffFit(align.indPoly+2)*proc.ppmVec + ...
                      coeffFit(align.indPoly+3);
        case 4
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^4 + ...
        ...
        case 5
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^5 + ...
        ...
        case 6
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^6 + ...
        ...
        case 7
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^7 + ...
        ...
        case 8
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^8 + ...
        ...
        case 9
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^9 + ...
        ...
        case 10
            polyFit = coeffFit(align.indPoly)*proc.ppmVec.^10 + ...
    ...
    if flag.verbose
        ...
        [minI,maxI,ppmZoom,spec2Zoom,f_done] = SP2_Proc_ExtractPpmRange(ppmMin,ppmMax,proc.ppmCalib,proc.spec1.sw,real(proc.spec1.spec));
        plot(ppmZoom,polyFit(minI:maxI),'r')
        hold off
    end
    
    %--- data modification ---
    proc.spec1.spec = proc.spec1.spec - polyFit;
...
fprintf('Min/max ratio difference vs. spectrum 1: %.2f%%\n',...
        100*(max(specDiffZoom)-min(specDiffZoom))/(max(spec1Zoom)-min(spec1Zoom)))
fprintf('Integral spec2/spec1/diff: %f/%f/%f\n',...
        sum(spec2Zoom),sum(spec1Zoom),sum(specDiffZoom))
fprintf('SD ratio difference vs. spectrum 1: %.2f%%\n\n',100*std(specDiffZoom)/std(spec1Zoom));
