%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function outdata = SP2_Proc_AlignSpec2FitFct(coeff,data)
%%
...
global loggingfile proc align flag



%--- data assignment: spectrum 1 ---
align.fid    = proc.spec2.fidOrig;
align.nspecC = proc.spec2.nspecCOrig;

%--- exponential line broadening ---
if flag.procSpec2Lb
    %--- generation of weighting function ---
    if flag.procAlignLb
        align.lbWeight = exp(-coeff(align.indLb)*proc.spec2.dwell*(0:align.nspecC-1)*pi)';
    else
        align.lbWeight = exp(-proc.spec2.lb*proc.spec2.dwell*(0:align.nspecC-1)*pi)';
    end
...
if flag.procSpec2Gb
    %--- generation of weighting function ---
    if flag.procAlignGb
        align.gbWeight = exp(-coeff(align.indGb)*(proc.spec2.dwell*(0:align.nspecC-1)*pi).^2)';
        % align.gbWeight = exp(-pi^2/(4*log(2)) * coeff(align.indGb)^2*(proc.spec2.dwell*(0:align.nspecC-1)).^2)';
    else
        align.gbWeight = exp(-proc.spec2.gb*(proc.spec2.dwell*(0:align.nspecC-1)*pi).^2)';
        % align.gbWeight = exp(-pi^2/(4*log(2)) * proc.spec2.gb^2*(proc.spec2.dwell*(0:align.nspecC-1)).^2)';
    end
...
if flag.procSpec2Shift
    if flag.procAlignShift
        align.fid = align.fid .* exp(1i*2*pi*(0:align.nspecC-1)' * proc.spec2.dwell * coeff(align.indShift));   % corr phase per point
    else
        align.fid = align.fid .* exp(1i*2*pi*(0:align.nspecC-1)' * proc.spec2.dwell * proc.spec2.shift);        % corr phase per point
    end
end
...
if flag.procSpec2Cut
    if proc.spec2.cut<align.nspecC
        align.fid    = align.fid(1:proc.spec2.cut,1);
        align.nspecC = proc.spec2.cut;
    end
end

%--- time-domaine zero-filling ---
if flag.procSpec2Zf
    if proc.spec2.zf>align.nspecC
        align.fid1Zf = complex(zeros(proc.spec2.zf,1));
        align.fid1Zf(1:align.nspecC,1) = align.fid;
        align.fid = align.fid1Zf;
        align.nspecC = proc.spec2.zf;
    end
end
...
if flag.procSpec2Offset
    if flag.procAlignOffset
        ...
        align.fid(1) = align.fid(1)*proc.spec2.offset;
    end
end
...
if flag.procSpec2Phc1
    if flag.procAlignPhc1
        ...
        if proc.spec2.phc1~=0
            align.phaseVec = (0:proc.spec2.phc1/(align.nspecC-1):proc.spec2.phc1); 
            align.spec = align.spec .* exp(1i*align.phaseVec*pi/180)';
        end
    end
end
...
if flag.procSpec2Scale
    if flag.procAlignScale
        ...
        align.spec = proc.spec2.scale * align.spec;
    end
end
...
if flag.procSpec2Stretch
    if flag.procAlignStretch
        align.frequVecNew = 1/(1+coeff(align.indStretch)/proc.spec2.sf)*align.frequVecOrig;
    else
        align.frequVecNew = 1/(1+proc.spec2.stretch/proc.spec2.sf)*align.frequVecOrig;
    end
