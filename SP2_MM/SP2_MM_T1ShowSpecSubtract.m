%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_done = SP2_MM_T1ShowSpecSubtract( f_new )
%%
...
FCTNAME = 'SP2_MM_T1ShowSpecSubract';
...
if ~isfield(mm,'fhT1SpecSubract') || ~ishandle(mm.fhT1SpecSubract)
    ...
    if flag.mmPpmShowPos
        set(mm.fhT1SpecSubract,'NumberTitle','off','Name',sprintf(' Multi-T1 Analysis: Longest saturation delay minus T1 selection (point %i, %.3f ppm)',mm.expPointSelect,mm.expPpmSelect),...
            'Position',[314 114 893 550],'Color',[1 1 1]);
    else
        set(mm.fhT1SpecSubract,'NumberTitle','off','Name',sprintf(' Multi-T1 Analysis: Longest saturation delay minus T1 selection'),...
            'Position',[314 114 893 550],'Color',[1 1 1]);
    end
else
...
if flag.mmTOneSpline
    tOneSpecSubtract = real(sum(mm.t1spec,2)) - smooth(sum(mm.t1spec(:,mm.tOneCons),2),7);
else
    tOneSpecSubtract = real(sum(mm.t1spec,2)) - sum(mm.t1spec(:,mm.tOneCons),2);
end
