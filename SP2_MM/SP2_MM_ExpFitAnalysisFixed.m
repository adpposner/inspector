%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function SP2_MM_ExpFitAnalysisFixed( f_new )
%%
%%  Analysis of multi-exponential fitting result for fixed T1s.
%% 
...
FCTNAME = 'SP2_MM_ExpFitAnalysisFixed';
...
for t1Cnt = 1:size(mm.t1spec,2)
    fprintf('T1 #%.0f: T1 %.3fs, amplitude %.1f\n',t1Cnt,...
            mm.t1fit(mm.expPointSelect,t1Cnt),mm.t1spec(mm.expPointSelect,t1Cnt)) 
end
fprintf('\n')
...
plot(mm.satRecFine,zeros(1,size(mm.satRecSpecFine,2)),'g')
hold off
if flag.mmAmplShow          % automatic
    ...
    plotLim(3) = -plotLim(4)/10;        % arbitrary value to include zero
else                        % direct
...
subplot(3,1,3)
hold on
plot(mm.t1fit(mm.expPointSelect,:),mm.t1spec(mm.expPointSelect,:))
plot(mm.t1fit(mm.expPointSelect,:),mm.t1spec(mm.expPointSelect,:),'r*')
[plotLim(1) plotLim(2) plotLim(3) plotLim(4)] = SP2_IdealAxisValuesXGap(mm.t1fit(mm.expPointSelect,:),mm.t1spec(mm.expPointSelect,:));
if plotLim(1)~=plotLim(2) && plotLim(3)~=plotLim(4)
    axis(plotLim)
end
