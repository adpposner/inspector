%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_done = SP2_Data_PhilipsSdatParsConversion(procpar,nData)
%% 
...
FCTNAME = 'SP2_Data_PhilipsSdatParsConversion';
...
if isfield(procpar,'scan_id')
    eval(['data.spec' num2str(nData) '.sequence = procpar.scan_id;'])
else
...
%     end
% end
if isfield(procpar,'samples')
    eval(['data.spec' num2str(nData) '.nspecC   = procpar.samples;'])
else
...
% if isfield(procpar,'averages')       % the inital NR is the sum of water and metab
%     eval(['data.spec' num2str(nData) '.nr = procpar.averages;'])
% else
...
% end
if isfield(procpar,'averages')
    eval(['data.spec' num2str(nData) '.na = procpar.averages;'])
else
...
if isfield(procpar,'synthesizer_frequency')
    eval(['data.spec' num2str(nData) '.sf = procpar.synthesizer_frequency/1e6;'])           % Larmor frequency in MHz
else
    eval(['data.spec' num2str(nData) '.sf = 0;'])           % Larmor frequency in MHz
end

if isfield(procpar,'offset_frequency')
    eval(['data.spec' num2str(nData) '.offset   = procpar.offset_frequency;'])                       % frequency offset in Hz
else
    eval(['data.spec' num2str(nData) '.offset   = 0;'])                                 % frequency offset in Hz
end
if isfield(procpar,'sample_frequency')                                                         % 
    eval(['data.spec' num2str(nData) '.sw_h  = procpar.sample_frequency;'])  % sweep width in Hz
    eval(['data.spec' num2str(nData) '.dwell = 1/procpar.sample_frequency;'])               % orig: [us]
...
if isfield(procpar,'repetition_time')
    eval(['data.spec' num2str(nData) '.tr = procpar.repetition_time;'])     % orig: [ms]
else
    eval(['data.spec' num2str(nData) '.tr = 0;'])
end
if isfield(procpar,'echo_time')
    eval(['data.spec' num2str(nData) '.te = procpar.echo_time;'])    % orig: [ms]
else
...
if isfield(procpar,'lr_off_center')
    eval(['data.spec' num2str(nData) '.pos(1) = procpar.lr_off_center;'])
else
    eval(['data.spec' num2str(nData) '.pos(1) = 0;'])
end
if isfield(procpar,'ap_off_center')
    eval(['data.spec' num2str(nData) '.pos(2) = procpar.ap_off_center;'])
else
    eval(['data.spec' num2str(nData) '.pos(2) = 0;'])
end
if isfield(procpar,'cc_off_center')
    eval(['data.spec' num2str(nData) '.pos(3) = procpar.cc_off_center;'])
else
    eval(['data.spec' num2str(nData) '.pos(3) = 0;'])
end
if isfield(procpar,'lr_size')
    eval(['data.spec' num2str(nData) '.vox(1) = procpar.lr_size;'])
else
    eval(['data.spec' num2str(nData) '.vox(1) = 0;'])
end
if isfield(procpar,'ap_size')
    eval(['data.spec' num2str(nData) '.vox(2) = procpar.ap_size;'])
else
    eval(['data.spec' num2str(nData) '.vox(2) = 0;'])
end
if isfield(procpar,'cc_size')
    eval(['data.spec' num2str(nData) '.vox(3) = procpar.cc_size;'])
else
