%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_done = SP2_Data_ProcparConversion(procpar,nData)
%% 
...
if isfield(procpar,'seqcon')
    eval(['data.spec' num2str(nData) '.seqcon = procpar.seqcon;'])
end
eval(['data.spec' num2str(nData) '.sequence = procpar.seqfil;'])
...
eval(['data.spec' num2str(nData) '.nspecC   = procpar.np/2;'])
eval(['data.spec' num2str(nData) '.nx       = 1;'])
eval(['data.spec' num2str(nData) '.ny       = 1;'])
% eval(['data.spec' num2str(nData) '.ns       = procpar.ct;'])         % note: not NS, NSLICES=PVM_NVoxels
if isfield(procpar,'nf')
    eval(['data.spec' num2str(nData) '.ni = procpar.nf;'])    
end
if isfield(procpar,'njde')
    eval(['data.spec' num2str(nData) '.njde = procpar.njde;'])    
end
...
if isfield(procpar,'nr')        % guarantee backward compatibility
    switch flag.dataExpType
        case 2          % single spectrum
            % also note the similarity of acqcycles to njde for edited experiments
            if procpar.saveSeparately       % FIDs saved separately to file
                % potential 1) phase cycling (separately saved) and 2) arrayed experiment
                eval(['data.spec' num2str(nData) '.nr = procpar.acqcycles;'])
            else                        % combined, i.e. single FID
                eval(['data.spec' num2str(nData) '.nr = procpar.acqcycles;'])
            end
        case 6          % MRSI
            % also note the similarity of acqcycles to njde for edited experiments
        ...
        otherwise
            ...
            if procpar.saveSeparately       % FIDs saved separately to file
                % potential 1) phase cycling (separately saved) and 2) arrayed experiment
                if strcmp(procpar.seqcon(3),'c')
                    eval(['data.spec' num2str(nData) '.nr = procpar.nv*procpar.acqcycles;'])
                else
                    eval(['data.spec' num2str(nData) '.nr = procpar.acqcycles;'])
                end
            else                        % combined, i.e. single FID
...
else                            % backward compatibility before separate saving existed
    % potentially arrayed experiment
    eval(['data.spec' num2str(nData) '.nr = procpar.acqcycles;'])
end
if iscell(procpar.nt)
    eval(['data.spec' num2str(nData) '.na = cell2mat(procpar.nt);'])
else
    eval(['data.spec' num2str(nData) '.na = procpar.nt;'])
end
eval(['data.spec' num2str(nData) '.nv       = procpar.nv;'])
eval(['data.spec' num2str(nData) '.sf       = procpar.sfrq;'])       % Larmor frequency in MHz
eval(['data.spec' num2str(nData) '.sw_h     = procpar.sw;'])         % sweep width in Hz
eval(['data.spec' num2str(nData) '.offset   = procpar.tof;'])        % frequency offset in Hz
eval(['data.spec' num2str(nData) '.dwell    = 1/procpar.sw;'])       % dwell time
eval(['data.spec' num2str(nData) '.sw       = procpar.sw/procpar.sfrq;'])    % sweep width in ppm
eval(['data.spec' num2str(nData) '.pp       = procpar.seqfil;'])
if strcmp(procpar.dp,'y')               % double precision
    eval(['data.spec' num2str(nData) '.wordSize = ''32bit'';'])
else
    eval(['data.spec' num2str(nData) '.wordSize = ''16bit'';'])
end
if isfield(procpar,'TR')
    eval(['data.spec' num2str(nData) '.tr = procpar.TR*1000;'])
elseif isfield(procpar,'tr')
    eval(['data.spec' num2str(nData) '.tr = procpar.tr*1000;'])
end
if isfield(procpar,'TE')
    if iscell(procpar.TE)
        eval(['data.spec' num2str(nData) '.te = cell2mat(procpar.TE)*1000;'])
    else
        eval(['data.spec' num2str(nData) '.te = procpar.TE*1000;'])
    end
elseif isfield(procpar,'te')
    eval(['data.spec' num2str(nData) '.te = procpar.te*1000;'])
else
    eval(['data.spec' num2str(nData) '.te = 0;'])
end
if isfield(procpar,'TM')
    eval(['data.spec' num2str(nData) '.tm = procpar.TM*1000;'])
else
    eval(['data.spec' num2str(nData) '.tm = 0;'])
end
if isfield(procpar,'pos1')
    eval(['data.spec' num2str(nData) '.pos(1) = procpar.pos1*10;'])
else
    eval(['data.spec' num2str(nData) '.pos(1) = 0;'])
end
if isfield(procpar,'pos2')
    eval(['data.spec' num2str(nData) '.pos(2) = procpar.pos2*10;'])
else
    eval(['data.spec' num2str(nData) '.pos(2) = 0;'])
end
if isfield(procpar,'pos3')
    eval(['data.spec' num2str(nData) '.pos(3) = procpar.pos3*10;'])
else
    eval(['data.spec' num2str(nData) '.pos(3) = 0;'])
end
if isfield(procpar,'vox1')
    eval(['data.spec' num2str(nData) '.vox(1) = procpar.vox1;'])
else
    eval(['data.spec' num2str(nData) '.vox(1) = 0;'])
end
if isfield(procpar,'vox2')
    eval(['data.spec' num2str(nData) '.vox(2) = procpar.vox2;'])
else
    eval(['data.spec' num2str(nData) '.vox(2) = 0;'])
end
if isfield(procpar,'vox3')
    eval(['data.spec' num2str(nData) '.vox(3) = procpar.vox3;'])
else
...
if any(strfind(data.spec1.sequence,'spuls'))    % spulse
    ...
    eval(['data.spec' num2str(nData) '.rf1.power   = procpar.tpwr;'])
end
...
if isfield(procpar,'TI')
    eval(['data.spec' num2str(nData) '.inv.ti = procpar.TI;'])
else
...
eval(['data.spec' num2str(nData) '.nRcvrs  = procpar.nRcvrs;'])
eval(['data.spec' num2str(nData) '.z0      = procpar.z0_;'])
eval(['data.spec' num2str(nData) '.yz      = procpar.yz;'])
eval(['data.spec' num2str(nData) '.xy      = procpar.xy;'])
eval(['data.spec' num2str(nData) '.x2y2    = procpar.x2y2;'])
eval(['data.spec' num2str(nData) '.x1      = procpar.x1;'])
eval(['data.spec' num2str(nData) '.x3      = procpar.x3;'])
eval(['data.spec' num2str(nData) '.y1      = procpar.y1;'])
eval(['data.spec' num2str(nData) '.xz      = procpar.xz;'])
eval(['data.spec' num2str(nData) '.xz2     = procpar.xz2;'])
eval(['data.spec' num2str(nData) '.y3      = procpar.y3;'])
eval(['data.spec' num2str(nData) '.z1c     = procpar.z1c;'])
eval(['data.spec' num2str(nData) '.yz2     = procpar.yz2;'])
eval(['data.spec' num2str(nData) '.z2c     = procpar.z2c;'])
eval(['data.spec' num2str(nData) '.z5      = procpar.z5;'])
eval(['data.spec' num2str(nData) '.z3c     = procpar.z3c;'])
eval(['data.spec' num2str(nData) '.z4c     = procpar.z4c;'])
eval(['data.spec' num2str(nData) '.zx2y2   = procpar.zx2y2;'])
eval(['data.spec' num2str(nData) '.zxy     = procpar.zxy;'])
...
if isfield(procpar,'time_run')          % ISO 8601 -> date vector
    eval(['data.spec' num2str(nData) '.time_start = datevec(procpar.time_run,''yyyymmddTHHMMSS'');'])
end
...
if isfield(procpar,'time_run') && isfield(procpar,'time_complete')
    eval(['data.spec' num2str(nData) '.durMin = (data.spec' num2str(nData) '.time_end(4)-data.spec' num2str(nData) '.time_start(4))*60 + (data.spec' num2str(nData) '.time_end(5)-data.spec' num2str(nData) '.time_start(5)) + (data.spec' num2str(nData) '.time_end(6)-data.spec' num2str(nData) '.time_start(6))/60;'])
    eval(['data.spec' num2str(nData) '.durSec = data.spec' num2str(nData) '.durMin*60;'])
end
