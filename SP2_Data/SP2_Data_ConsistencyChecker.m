%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
function [f_err,msg] = SP2_Data_ConsistencyChecker(callername,meth1,meth2)
    %%
    %%  function [f_err,msg] = SP2_Data_ConsistencyChecker(meth1,meth2)
    ...
    %%
    %%  01-2023 Russell Posner
    %%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    FCTNAME = 'SP2_Data_ConsistencyChecker';
...
if meth1No==meth2No         % struct sizes are the same
    ...
    if pattNo==0            % struct fields are same
                        ...
                        if ~any(size(par1)~=size(par2))
                            if par1~=par2
                                if ~strcmp('sfrq',char(meth1Names(icnt))) && ~strcmp('tof',char(meth1Names(icnt)))
                                    msg = SP2_Data_InconsistencyMessage(callername,msg,char(meth1Names(icnt)),par1,par2);
                                    %msg = sprintf('sfrq/tof: %s%s is not consistent\n',msg,char(meth1Names(icnt)));
                                    f_err = 1;
                                end
                            end
                        else
                            f_err = 1;
                            msg = sprintf('%s: any size: %s%s is not consistent\n',callername,msg,char(meth1Names(icnt)));
                        end
                    else
                        f_err = 1;
                        msg = sprintf('%s: ndims: %s%s is not consistent\n',callername,msg,char(meth1Names(icnt)));
                    end
                else
                    f_err = 1;
                    msg = sprintf('%s: nonnumeric: %s%s is not consistent\n',callername,msg,char(meth1Names(icnt)));
                end
            elseif isstruct(par1)
                if isstruct(par2)
                    [f_err,msg] = SP2_Data_ConsistencyChecker(callername,par1,par2);       %...
                else
                    f_err = 1;
                    msg = sprintf('%s: %s%s is not consistent\n',callername, msg,char(meth1Names(icnt)));
                end
            else
                if ~strcmp(par1,par2)
                    msg = sprintf('%s: %s%s is not consistent\n',callername, msg,char(meth1Names(icnt)));
        ...
        f_err = 1;
        msg = sprintf('%s: %s -> %.0f method struct fields differ:\n',FCTNAME,pattNo);
        for icnt = 1:pattNo
