%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function [dataSpec, f_succ] = SP2_Data_SiemensDatReadFidVDVE(dataSpec)
%%
...
FCTNAME = 'SP2_Data_SiemensDatReadFidVDVE';
...
if isempty(h.ulFlagsAndDMALength)
    fprintf(loggingfile,'\n%s ->\nUnknown data format.\nCheck with Christoph for solutions (cwj2112@columbia.edu)\n\n',FCTNAME)
    return
end
...
% if 1
    ...
    fprintf(loggingfile,'debugOffset      = %.1f bytes (= %.0f pts at 8 bytes / point)\n',...
            debugOffset/8,debugOffset)
    fprintf(loggingfile,'ushSamplesInScan = %.0f\n',h.ushSamplesInScan)
else
    debugOffset = 0;
end
...
%                     ((nrCnt-1)*dataSpec.nRcvrs+(chCnt-1))*((2*dataSpec.nspecC)*8)+8;    % before FID data
        ...
        if nrCnt-dataSpec.dsOffset>dataSpec.nr || size(data,1)~=2*h.ushSamplesInScan
            ...
            fprintf(loggingfile,'nrCnt = %.0f, chCnt = %.0f, dsOffset = %.0f\n',nrCnt,chCnt,dataSpec.dsOffset);
        else
...
%     end
    ...
    % dataSpecTmp  = ifft(circshift(dataSpec.fid,[ptShift 0 0]),[],1);    % odd a problem for quadrant below?
...
% dataSpec.nspecC = dataSpec.nspecC*dataSpec.overSamplFac;
    
%--- info printout ---
fprintf(loggingfile,'%s ->\nData file successfully read\n',FCTNAME);
fprintf(loggingfile,'nr/nspecC/nRcvrs: %d+%d/%d/%d\n',dataSpec.trOffset,...
