%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_done = SP2_Data_DicomImaParsConversion(procpar,nData)
%% 
%%  Conversion of DICOM procpar parameters to method structure to be used 
%%  in this software.
%%
...
FCTNAME = 'SP2_Data_DicomImaParsConversion';
...
if isfield(procpar,'Manufacturer')                  % manufacturer
    eval(['data.spec' num2str(nData) '.dcmManu = procpar.Manufacturer;'])
    fprintf('Manufacturer: %s\n',procpar.Manufacturer)
else
    fprintf('\n%s ->\nEssential DICOM entry <Manufacturer> not found. Program aborted.\n',FCTNAME)
    return
end

if eval(['strfind(data.spec' num2str(nData) '.dcmManu,''Philips'')'])

    %--- manufacturer ---
    eval(['data.spec' num2str(nData) '.dcmManu = ''Philips'';'])        % actual entry might be longer
    
    %--- parameter transfer to 'method' struct ---
    if isfield(procpar,'PulseSequenceName')
        eval(['data.spec' num2str(nData) '.sequence = procpar.PulseSequenceName;'])
    else
    ...
    if isfield(procpar,'DataPointColumns')
        eval(['data.spec' num2str(nData) '.nspecC = procpar.DataPointColumns;'])
    else
        eval(['data.spec' num2str(nData) '.nspecC = '''';'])
    end
    
%     if isfield(procpar,'procpar.Private_2005_1371')             % nspecC
%         if isfield(procpar.procpar.Private_2005_1371,'Item_1')      
%            if isfield(procpar.procpar.Private_2005_1371.Item_1,'Private_2005_1372')
%                 eval(['data.spec' num2str(nData) '.nspecC = procpar.procpar.Private_2005_1371.Item_1.Private_2005_1372;'])
%            else
...
%     end
    ...
    % end
    % if isfield(procpar,'nex')
    %     eval(['data.spec' num2str(nData) '.nr = procpar.nex;'])
    % else
    ...
    % end
    if isfield(procpar,'Private_2001_1088')                 
        eval(['data.spec' num2str(nData) '.nr = procpar.Private_2001_1088;'])
    else
        eval(['data.spec' num2str(nData) '.nr = 1;'])
    end
    ...
    if isfield(procpar,'Private_2001_1083')
        eval(['data.spec' num2str(nData) '.sf = procpar.Private_2001_1083;'])             % Larmor frequency in MHz
    end
    ...
    if isfield(procpar,'Private_2005_1338')             % bandwidth [Hz]
        eval(['data.spec' num2str(nData) '.dwell = procpar.Private_2005_1338;'])                    % sweep width in Hz
    ...
    if isfield(procpar,'Private_2005_1357')
        eval(['data.spec' num2str(nData) '.tr = procpar.Private_2005_1357;'])                  % orig: [ms]
    else
        eval(['data.spec' num2str(nData) '.tr = 0;'])
    end
    if isfield(procpar,'Private_2005_1360')
        eval(['data.spec' num2str(nData) '.te = procpar.Private_2005_1360;'])      % orig: [ms]
    else
    ...
    if isfield(procpar,'Private_2001_105f')          % voxel size
        if isfield(procpar.Private_2001_105f,'Item_1')      
           if isfield(procpar.Private_2001_105f.Item_1,'Private_2005_1074')
                eval(['data.spec' num2str(nData) '.vox(1) = procpar.Private_2001_105f.Item_1.Private_2005_1074;'])
           else
                eval(['data.spec' num2str(nData) '.vox(1) = 0;'])
           end
           if isfield(procpar.Private_2001_105f.Item_1,'Private_2005_1075')
                eval(['data.spec' num2str(nData) '.vox(2) = procpar.Private_2001_105f.Item_1.Private_2005_1075;'])
           else
                eval(['data.spec' num2str(nData) '.vox(2) = 0;'])
           end
           if isfield(procpar.Private_2001_105f.Item_1,'Private_2005_1076')
                eval(['data.spec' num2str(nData) '.vox(3) = procpar.Private_2001_105f.Item_1.Private_2005_1076;'])
           else
    ...
    if isfield(procpar,'Private_2001_105f')          % voxel position
        if isfield(procpar.Private_2001_105f,'Item_1')      
           if isfield(procpar.Private_2001_105f.Item_1,'Private_2005_1078')
                eval(['data.spec' num2str(nData) '.pos(1) = procpar.Private_2001_105f.Item_1.Private_2005_1078;'])
           else
                eval(['data.spec' num2str(nData) '.pos(1) = 0;'])
           end
           if isfield(procpar.Private_2001_105f.Item_1,'Private_2005_1079')
                eval(['data.spec' num2str(nData) '.pos(2) = procpar.Private_2001_105f.Item_1.Private_2005_1079;'])
           else
                eval(['data.spec' num2str(nData) '.pos(2) = 0;'])
           end
           if isfield(procpar.Private_2001_105f.Item_1,'Private_2005_107a')
                eval(['data.spec' num2str(nData) '.pos(3) = procpar.Private_2001_105f.Item_1.Private_2005_107a;'])
           else
    ...
    if isfield(procpar,'FileModDate')          % -> date vector
        eval(['data.spec' num2str(nData) '.time_start = datevec(procpar.FileModDate);'])
    end
    ...
    % end
elseif eval(['any(strfind(data.spec' num2str(nData) '.dcmManu,''Siemens''))']) || ...
       eval(['any(strfind(data.spec' num2str(nData) '.dcmManu,''SIEMENS''))'])       % Siemens

    %--- manufacturer ---
    eval(['data.spec' num2str(nData) '.dcmManu = ''Siemens'';'])        % actual entry might be longer
    ...
    if isfield(procpar,'ProtocolName')
        eval(['data.spec' num2str(nData) '.sequence = procpar.ProtocolName;'])
    else
        eval(['data.spec' num2str(nData) '.sequence = '''';'])
    end
    % if any(strfind(data.spec1.sequence,'JDE'))    % STEAM
    ...
    %     end
    % end
    if isfield(procpar,'frame_size')
        eval(['data.spec' num2str(nData) '.nspecC   = procpar.frame_size;'])
    end
    if isfield(procpar,'CSIMatrixSize')
        eval(['data.spec' num2str(nData) '.nx       = procpar.CSIMatrixSize(1);'])
        eval(['data.spec' num2str(nData) '.ny       = procpar.CSIMatrixSize(2);'])
    ...
    if isfield(procpar,'NumberOfAverages')
        eval(['data.spec' num2str(nData) '.nr = procpar.NumberOfAverages;'])
    else
        eval(['data.spec' num2str(nData) '.nr = 1;'])
    end
    ...
    if isfield(procpar,'DwellTime')                                                         % double precision
        eval(['data.spec' num2str(nData) '.dwell = procpar.DwellTime;'])                    % sweep width in Hz
        eval(['data.spec' num2str(nData) '.sw_h  = 1/data.spec' num2str(nData) '.dwell'])   % sweep width in Hz
        eval(['data.spec' num2str(nData) '.sw    = data.spec' num2str(nData) '.sw_h/data.spec' num2str(nData) '.sf;'])         % sweep width in Hz
    else
        eval(['data.spec' num2str(nData) '.sw_h  = 0;'])
    ...
    fprintf('\n%s ->\nManufacturer not recognized. Program aborted.\n',FCTNAME)
end
