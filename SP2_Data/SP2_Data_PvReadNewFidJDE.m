%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function [datFid, f_done] = SP2_Data_PvReadNewFidJDE(dataSpec)
%%
...
FCTNAME = 'SP2_Data_PvReadNewFidJDE';
...
if ( fsize/NumBytes == (2*nspecC)*nx*ny*ns*ni*nr*njde)
    fprintf('reading <%s>\n', dataSpec.fidFile);
...
datFid  = double(reshape(datFid,2,nspecC*nx*ny*ns*ni*nr*njde));
datImag = datFid(2,:);
datReal = datFid(1,:);
datFid  = complex(datReal, datImag);
if nx==1 && ny==1 && ns==1 && ni==1
    datFid  = reshape(datFid,nspecC,1,nr*njde);      % include singleton nRcvrs dimension for later data handling
else
