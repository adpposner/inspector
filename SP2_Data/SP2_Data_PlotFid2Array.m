%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_succ = SP2_Data_PlotFid2Array( f_new )
%%
...
FCTNAME = 'SP2_Data_PlotFid2Array';
...
if dataSelectN*data.rcvrN>data.plotArrMaxElem
    fprintf('\n*** WARNING ***\n')
    fprintf('Figure update of Fid 2 array has been temporarily disabled\nsince requested array size has become unreasonable (%.0f>%.0f).\n\n',...
...
if f_new && isfield(data,'fhFid2Array') && ~flag.procKeepFig
    if ishandle(data.fhFid2Array)
        delete(data.fhFid2Array)
    end
    data = rmfield(data,'fhFid2Array');
end
% create figure if necessary
if ~isfield(data,'fhFid2Array') || ~ishandle(data.fhFid2Array)
    data.fhFid2Array = figure('IntegerHandle','off');
    set(data.fhFid2Array,'NumberTitle','off','Name',sprintf(' FID 2: Array'),...
        'Position',[650 45 560 550],'Color',[1 1 1]);
else
    set(0,'CurrentFigure',data.fhFid2Array)
    if flag.dataKeepFig
        if ~SP2_KeepFigure(data.fhFid2Array)
            return
        end
    end
end
clf(data.fhFid2Array)
    ...
    for repCnt = 1:dataSelectN       % number of selected spectra
        ...
        if data.spec2.dim==2    % 2D
            if data.spec2.nspecC<data.fidCut        % no apodization
                datFid = data.spec2.fid(:,data.rcvrInd(rcvrCnt));
            else                                    % apodization
                datFid = data.spec2.fid(1:data.fidCut,data.rcvrInd(rcvrCnt));
            ...
            if data.spec2.nspecC<data.fidCut        % no apodization
                datFid = data.spec2.fid(:,data.rcvrInd(rcvrCnt),dataSelect(repCnt));
            else                                    % apodization
                datFid = data.spec2.fid(1:data.fidCut,data.rcvrInd(rcvrCnt),dataSelect(repCnt));
                if rcvrCnt==1 && repCnt==1
                    fprintf('%s ->\nApodization of FID 2 to %.0f points applied.\n',FCTNAME,data.fidCut)
                end
            end
