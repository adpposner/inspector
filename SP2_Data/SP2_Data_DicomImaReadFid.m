%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function [dataSpec,f_succ] = SP2_Data_DicomImaReadFid(dataSpec)
%%
...
FCTNAME = 'SP2_Data_DicomImaReadFid';
...
%--- open .IMA file ---
fileId = fopen(dataSpec.fidFile,'rb');
if fileId<0 
...
%--- check for DICOM label in header ---
hdr    = fread(fileId,128,'uchar');             % skip beginning
dcmStr = fread(fileId,4,'*char')';              % read alleged DIC(O)M entry
if ~strcmp(dcmStr,'DICM')
    fprintf('%s ->\nAssigned file is not a DICOM file. Program aborted.',FCTNAME);
    fprintf('%s\n',dataSpec.fidFile);
    fclose(fileId);
    return
end

%--- advance file pointer to data start ---
[fieldBytes,f_done] = dicom_move_loc(fileId,'7FE1','1010');          % pointer positioning, return value: field size [bytes]
if ~f_done
    fprintf('%s ->\nData file handling failed. Program aborted.\n',FCTNAME)
    return
end
fieldLen   = fieldBytes / 4;                                % field length at 4 bytes per value (ieee-le)
[fidRaw,readLen] = fread(fileId,fieldLen,'float32','ieee-le');
 
%--- consistency check ---
if fieldLen~=readLen
    fprintf('%s ->\nIncomplete data read from file (%d of %d values). Program aborted.\n',...
            FCTNAME,fieldLen,readLen);
    return
end

%--- reformat to complex ---
dataSpec.fid    = complex(fidRaw(1:2:end),-fidRaw(2:2:end));
dataSpec.nspecC = size(dataSpec.fid,1);
...
function [length, f_succLoc] = dicom_move_loc(fd, strGroup, strElement)
