%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function [datFid, f_done] = SP2_Data_PvReadNewRawDataJDE(dataSpec)
%%
...
FCTNAME = 'SP2_Data_PvReadNewRawDataJDE';
...
if ( fsize/NumBytes == (2*nspecC)*nRcvrs*nx*ny*ns*ni*nr*njde)
    fprintf('%s\treading <%s>\n',mfilename, dataSpec.fidFile);
    fprintf('nspec/nRcvrs/nx/ny/ns/ni/nr/njde: %d/%d/%d/%d/%d/%d/%d/%d filesize %dk\n', ...
            2*nspecC,nRcvrs,nx,ny,ns,ni,nr,njde,fsize/1024);
else
    fprintf('%s: nint %d ~= nspec*nRcvrs*nx*ny*ni*ns*nr*njde = %d*%d*%d*%d*%d*%d*%d*%d = %d\n', ...
            FCTNAME,fsize/NumBytes,2*nspecC,nRcvrs,nx,ny,ns,ni,nr,njde,2*nspecC*nRcvrs*nx*ny*ns*ni*nr*njde);
    return
end
...
if fid <= 0   
    fprintf('%s -> Opening %s failed; %s',FCTNAME,dataSpec.fidFile,message);
    return
end
...
st = fclose(fid);
if st~=0
    fprintf('%s -> Closing file %s failed',dataSpec.fidFile);
    return
end

datFid  = double(reshape(datFid,2,nspecC*nRcvrs*nx*ny*ns*ni*nr*njde));
datImag = datFid(2,:);
datReal = datFid(1,:);
datFid  = complex(datReal, datImag);
if nx==1 && ny==1 && ns==1 && ni==1
    datFid  = squeeze(reshape(datFid,nspecC,nRcvrs,nr,njde));
else
    fprintf('Inconsistent data dimensions detected. Loading aborted.\n');
    fprintf('Hint: Check nx, ny, ni and ns\n\n');
    return
end
fprintf('resorting to complex: dim nspecC/nRcvrs/nx/ny/ns/ni/nr/njde: %d/%d/%d/%d/%d/%d/%d/%d \n',...
         nspecC,nRcvrs,nx,ny,ns,ni,nr,njde);
...
if nRcvrs>1
    phaseCorr = repmat(phaseCorr,1,nRcvrs,nr*njde);
else
    phaseCorr = repmat(phaseCorr,1,nr*njde);
end
