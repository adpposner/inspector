%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_succ = SP2_Data_PhaseCorrKloseRxComb
%% 
...
FCTNAME = 'SP2_Data_PhaseCorrKloseRxComb';
...
%--- NR range selection ---
nrRg = [1 data.nPhCycle];           % default NR range to be used for Klose correction
fprintf('Phase cycle combined for Klose correction: %i..%i\n',nrRg(1),nrRg(2));

%--- consistency check ---
if mod(data.spec2.nr,data.nPhCycle)~=0 
    fprintf('%s ->\nmod(NR=%i,nPC)~=0. Program aborted.\nChose ''Separate'' option instead.\n',...
...
refRx           = data.rcvrInd(maxInd);                 % reference Rx number (choose as strongest of the selected ones)

%--- work on local copy ---
dataSpec2Fid = data.spec2.fid;
...
anaPars.frAlignFrequRg  = 15;           % frequency alignment: potential frequency variation, +/- frequRg
...
for nrCnt = nrRg(1):nrRg(2)
    ...
    if nrCnt<nrRg(1)+5                  % verbose only for the first 4 FIDs
        [frequVec(nrCnt),f_succ] = SP2_Data_AutoFrequDetIndiv(dataSpec2Fid(:,refRx,nrCnt),dataSpec2Fid(:,refRx,nrRg(1)),data.spec2,flag.dataAlignVerbose,anaPars);
    else
        [frequVec(nrCnt),f_succ] = SP2_Data_AutoFrequDetIndiv(dataSpec2Fid(:,refRx,nrCnt),dataSpec2Fid(:,refRx,nrRg(1)),data.spec2,0,anaPars);
    end
    if ~f_succ
    ...
    for rxCnt = 1:data.rcvrN
        dataSpec2Fid(:,data.rcvrInd(rxCnt),nrCnt) = exp(-1i*phPerPt*(0:data.spec2.nspecC-1)') .* ...
                                                      squeeze(dataSpec2Fid(:,data.rcvrInd(rxCnt),nrCnt));
    end
end
%--- info printout ---
...
for nrCnt = nrRg(1):nrRg(2)
    %--- frequency shift determination with respect to first acquisition of each Rx channel ---
    if nrCnt<nrRg(1)+5            % verbose only for the first 4 FIDs
        [phaseVec(nrCnt),f_succ] = SP2_Data_AutoPhaseDet(dataSpec2Fid(:,refRx,nrCnt),data.spec2,flag.dataAlignVerbose,dataSpec2Fid(:,refRx,nrRg(1)),anaPars);
    else
        [phaseVec(nrCnt),f_succ] = SP2_Data_AutoPhaseDet(dataSpec2Fid(:,refRx,nrCnt),data.spec2,0,dataSpec2Fid(:,refRx,nrRg(1)),anaPars);
    end
    if ~f_succ
    ...
    for rxCnt = 1:data.rcvrN
        dataSpec2Fid(:,data.rcvrInd(rxCnt),nrCnt) = exp(1i*phaseVec(nrCnt)*pi/180) * ...
                                                      dataSpec2Fid(:,data.rcvrInd(rxCnt),nrCnt);                                          
    end
end
%--- info printout ---
...
data.spec2.kloseFid = sum(dataSpec2Fid(:,data.rcvrInd,nrRg(1):nrRg(2)),3);
