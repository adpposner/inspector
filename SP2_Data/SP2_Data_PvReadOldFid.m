%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function [datFid, f_done] = SP2_Data_PvReadOldFid(dataSpec)
%%
...
if ( fsize/NumBytes == (2*nspecC)*nx*ny*ns*ni*nr)
    fprintf('reading <%s>\n', dataSpec.fidFile);
    fprintf('nspec/nx/ny/ns/ni/nr: %d/%d/%d/%d/%d/%d filesize %dk\n', ...
            2*nspecC,nx,ny,ns,ni,nr,fsize/1024);
else
    fprintf('%s: nint %d ~= nspec*nx*ny*ni*ns*nr = %d*%d*%d*%d*%d*%d = %d\n', ...
            FCTNAME,fsize/NumBytes,2*nspecC,nx,ny,ns,ni,nr,2*nspecC*nx*ny*ns*ni*nr);
    return
end
...
datFid  = double(reshape(datFid,2,nspecC*nx*ny*ns*ni*nr));
datImag = datFid(2,:);
datReal = datFid(1,:);
datFid  = complex(datReal, datImag);
% datFid  = reshape(datFid,nspecC,nx,ny,ns,ni,nr);
if nx==1 && ny==1 && ns==1 && ni==1
    datFid  = reshape(datFid,nspecC,nr);
else
    fprintf('Inconsistent data dimensions detected. Loading aborted.\n');
    fprintf('Hint: Check nx, ny, ni and ns\n\n');
    return
end
fprintf('resorting to complex: dim nspecC/nx/ny/ns/ni/nr: %d/%d/%d/%d/%d/%d \n',...
         nspecC,nx,ny,ns,ni,nr);
