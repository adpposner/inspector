%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_done = SP2_Data_PhaseCorrKloseRxCombJdeArray
%% 
...
if length(size(data.spec2.fidSelect))==3
    weightVec = mean(abs(data.spec2.fidSelect(1:5,data.rcvrInd,1)));
else
    weightVec = mean(abs(data.spec2.fidSelect(1:5,data.rcvrInd)));
end
...
for nrCnt = nrRg(1):nrRg(2)
    ...
    if nrCnt<nrRg(1)+5         % verbose only for the first 4 FIDs
        [frequVec(nrCnt),f_succ] = SP2_Data_AutoFrequDetIndiv(data.spec2.fidSelect(:,refRx,nrCnt),data.spec2.fidSelect(:,refRx,nrRg(1)),data.spec2,flag.dataAlignVerbose,anaPars);
    else
        [frequVec(nrCnt),f_succ] = SP2_Data_AutoFrequDetIndiv(data.spec2.fidSelect(:,refRx,nrCnt),data.spec2.fidSelect(:,refRx,nrRg(1)),data.spec2,0,anaPars);
    end
    if ~f_succ
    ...
    for rxCnt = 1:data.rcvrN
        data.spec2.fidSelect(:,data.rcvrInd(rxCnt),nrCnt) = exp(-1i*phPerPt*(0:data.spec2.nspecC-1)') .* ...
                                                      squeeze(data.spec2.fidSelect(:,data.rcvrInd(rxCnt),nrCnt));
    end
end
%--- info printout ---
...
for nrCnt = nrRg(1):nrRg(2)
    %--- frequency shift determination with respect to first acquisition of each Rx channel ---
    if nrCnt<nrRg(1)+5            % verbose only for the first 4 FIDs
        [phaseVec(nrCnt),f_succ] = SP2_Data_AutoPhaseDet(data.spec2.fidSelect(:,refRx,nrCnt),data.spec2,flag.dataAlignVerbose,data.spec2.fidSelect(:,refRx,nrRg(1)),anaPars);
    else
        [phaseVec(nrCnt),f_succ] = SP2_Data_AutoPhaseDet(data.spec2.fidSelect(:,refRx,nrCnt),data.spec2,0,data.spec2.fidSelect(:,refRx,nrRg(1)),anaPars);
    end
    if ~f_succ
    ...
    for rxCnt = 1:data.rcvrN
        data.spec2.fidSelect(:,data.rcvrInd(rxCnt),nrCnt) = exp(1i*phaseVec(nrCnt)*pi/180) * ...
                                                      data.spec2.fidSelect(:,data.rcvrInd(rxCnt),nrCnt);                                          
    end
end
%--- info printout ---
...
data.spec2.kloseFid = sum(data.spec2.fidSelect(:,data.rcvrInd,nrRg(1):nrRg(2)),3);
