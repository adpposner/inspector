...
FCTNAME = 'SP2_Data_QualitySelectInitJde1';
...
if flag.dataAllSelect               % all NR
    ...
    for seriesCnt = 1:data.spec1.seriesN
        if seriesCnt==1
            data.quality.selectStr = [num2str(first) ':2:' num2str(last)];
        else
            data.quality.selectStr = [data.quality.selectStr ' ' num2str(first+(seriesCnt-1)*data.spec1.nr) ':2:' num2str(last+(seriesCnt-1)*data.spec1.nr)];
        end
    end
else                                % selected NR
    %--- consistency checks ---
