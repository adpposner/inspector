...
%% 
%%  Init selection string with non-JDE experiments.
%%
...
FCTNAME = 'SP2_Data_QualitySelectInitNonJde';
...
if flag.dataAllSelect               % all NR
    ...
    for seriesCnt = 1:data.spec1.seriesN
        if seriesCnt==1
            data.quality.selectStr = [num2str(first) ':' num2str(last)];
        else
            data.quality.selectStr = [data.quality.selectStr ' ' num2str(first+(seriesCnt-1)*data.spec1.nr) ':' num2str(last+(seriesCnt-1)*data.spec1.nr)];
        end
    end
else
    %--- consistency checks ---
    ...
    if length(find(data.selectStr==':'))==1         % single colon: single, continuous group of NR
        ...
        for seriesCnt = 1:data.spec1.seriesN
            if seriesCnt==1
                data.quality.selectStr = num2str(first);
            else
                data.quality.selectStr = [data.quality.selectStr ' ' num2str(first+(seriesCnt-1)*data.spec1.nr)];
            end
        end
    end
end
