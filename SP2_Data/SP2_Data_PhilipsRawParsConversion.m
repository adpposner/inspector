%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_done = SP2_Data_PhilipsRawParsConversion(procpar,nData)
%% 
...
FCTNAME = 'SP2_Data_PhilipsRawParsConversion';


% nr_measurements = [32 4]          for 32 metab + 4 water
% nr_measured_channels = 32         nRcvrs
% total_nr_profiles = 1152          = (32 + 4) spec * 32 rcvrs
...
if isfield(procpar,'datStart')
    eval(['data.spec' num2str(nData) '.datStart = procpar.datStart;'])
else
    eval(['data.spec' num2str(nData) '.datStart = 0;'])
end

%--- parameter transfer to 'method' struct ---
if isfield(procpar,'scan_name')
    eval(['data.spec' num2str(nData) '.sequence = procpar.scan_name;'])
else
...
%     end
% end
if isfield(procpar,'recon_resolutions')
    % eval(['data.spec' num2str(nData) '.nspecC   = procpar.dr_max_encoding_numbers(1);'])
    eval(['data.spec' num2str(nData) '.nspecCOver = 8*procpar.recon_resolutions(1);'])
    eval(['data.spec' num2str(nData) '.nspecC     = procpar.recon_resolutions(1);'])
else
    eval(['data.spec' num2str(nData) '.nspecCOver = 0;'])
...
% end
if isfield(procpar,'nr_mixes')
    eval(['data.spec' num2str(nData) '.njde = procpar.nr_mixes;'])    
end
...
% end
if isfield(procpar,'nr_groups')
    eval(['data.spec' num2str(nData) '.nr_groups = procpar.nr_groups;'])
else
    eval(['data.spec' num2str(nData) '.nr_groups = 1;'])
end
% note the difference nr_measurements vs. nr
if isfield(procpar,'nr_measurements')       % the inital NR is the sum of water and metab
    eval(['data.spec' num2str(nData) '.nr = sum(procpar.nr_measurements);'])
    if length(procpar.nr_measurements)>1        % water comes first
        eval(['data.spec' num2str(nData) '.trOffset = procpar.nr_measurements(2);'])
    else
        eval(['data.spec' num2str(nData) '.trOffset = 0;'])
    end
else
    eval(['data.spec' num2str(nData) '.nr = 1;'])
    eval(['data.spec' num2str(nData) '.tr_offset = 0;'])
end
...
if isfield(procpar,'MRFrequency')
    ...
    eval(['data.spec' num2str(nData) '.sf = 0;'])                     % Larmor frequency in MHz
end
...
if isfield(procpar,'spectro_rep_times')
    eval(['data.spec' num2str(nData) '.tr = procpar.spectro_rep_times;'])     % orig: [ms]
else
    eval(['data.spec' num2str(nData) '.tr = 0;'])
end
if isfield(procpar,'spectro_echo_times')
    eval(['data.spec' num2str(nData) '.te = procpar.spectro_echo_times;'])    % orig: [ms]
else
...
if isfield(procpar,'location_center_coordinates')
    eval(['data.spec' num2str(nData) '.pos(1) = procpar.location_center_coordinates(1);'])
    eval(['data.spec' num2str(nData) '.pos(2) = procpar.location_center_coordinates(2);'])
    eval(['data.spec' num2str(nData) '.pos(3) = procpar.location_center_coordinates(3);'])
else
    eval(['data.spec' num2str(nData) '.pos(1) = 0;'])
    eval(['data.spec' num2str(nData) '.pos(2) = 0;'])
    eval(['data.spec' num2str(nData) '.pos(3) = 0;'])
end
if isfield(procpar,'voxel_sizes')
    eval(['data.spec' num2str(nData) '.vox(1) = procpar.voxel_sizes(1);'])
    eval(['data.spec' num2str(nData) '.vox(2) = procpar.voxel_sizes(2);'])
    eval(['data.spec' num2str(nData) '.vox(3) = procpar.voxel_sizes(3);'])
else
...
if isfield(procpar,'nr_measured_channels')
    eval(['data.spec' num2str(nData) '.nRcvrs = procpar.nr_measured_channels;'])
else
    eval(['data.spec' num2str(nData) '.nRcvrs = 1;'])
end
if isfield(procpar,'start_scan_date_time')       % -> date vector
    eval(['data.spec' num2str(nData) '.scan_date = procpar.start_scan_date_time;'])
    eval(['data.spec' num2str(nData) '.time_start = datevec(procpar.start_scan_date_time);'])
end
