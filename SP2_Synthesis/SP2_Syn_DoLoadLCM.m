%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function  f_succ = SP2_Syn_DoLoadLCM
%%
%%  Load spectrum from LCM page.
%%
...
if isfield(lcm,'expt')
    if isfield(lcm.expt,'fid')
        %--- format handling ---
        syn.fidOrig = lcm.expt.fid;
        syn.sf      = lcm.expt.sf;
        syn.sw_h    = lcm.expt.sw_h;
        syn.sw      = lcm.expt.sw_h/syn.sf;
...
%--- info printout ---
fprintf('FID loaded from LCM page\n');
