...
lcmDataDir = '/home/mrs2/JodiHippo/';                                       % LCModel data directory on analysis computer
lcmDir     = '/home/mrs2/';                                                 % LCModel software directory on analysis computer
    ...
    if f_procSLaserMetab                % process sLASER metabolites scan
        for dirCnt = 1:dirLen
            if any(strncmp(dirStruct(dirCnt).name,sprintf('%02i_meas_',numSLaserMetab),8) & ...
                ...
                fprintf(unit, ' FILRAW=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.raw',lcmDataDir,studyVec(sCnt),scanName));
                fprintf(unit, ' FILH2O=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.h2o',lcmDataDir,studyVec(sCnt),scanName));
                fprintf(unit, ' FILBAS=''%s''\n', sprintf('%sBasis/slaser_40.basis',lcmDataDir));
                fprintf(unit, ' FILTAB=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.table',lcmDataDir,studyVec(sCnt),scanName));
                fprintf(unit, ' FILPRI=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.dump',lcmDataDir,studyVec(sCnt),scanName));
                fprintf(unit, ' FILCSV=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.csv',lcmDataDir,studyVec(sCnt),scanName));
                fprintf(unit, ' FILPS=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.ps',lcmDataDir,studyVec(sCnt),scanName));
                ...
                controlFilePathLCModel = sprintf('%sAnalysis/LCModel_%03i/%s.control',lcmDataDir,studyVec(sCnt),scanName);
                fprintf(unit, '%s.lcmodel/bin/lcmodel < %s\n',controlFilePathLCModel,lcmDir);
        ...
        for jdeCnt = numJdeMetab(1):numJdeMetab(2)  % note that this is the expected file structure/nomenclature!
            ...
            for dirCnt = 1:dirLen
                if any(strncmp(dirStruct(dirCnt).name,sprintf('%02i_meas_',jdeCnt),8) & ...
                    ...
                    if isfield(proc,'fhSpecSuper')
                        if ishandle(proc.fhSpecSuper)
                            alignFigJpgPath = [scanResultDir 'SPX_SpecAlignResult_JDE.jpg'];
                            saveas(proc.fhSpecSuper,alignFigJpgPath,'jpg');
                        end
                    end
                    ...
                    fprintf(unit, ' FILRAW=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.raw',lcmDataDir,studyVec(sCnt),scanName));
                    fprintf(unit, ' FILH2O=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.h2o',lcmDataDir,studyVec(sCnt),scanName));
                    fprintf(unit, ' FILBAS=''%s''\n', sprintf('%sBasis/mslaser_70.basis',lcmDataDir));
                    fprintf(unit, ' FILTAB=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.table',lcmDataDir,studyVec(sCnt),scanName));
                    fprintf(unit, ' FILPRI=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.dump',lcmDataDir,studyVec(sCnt),scanName));
                    fprintf(unit, ' FILCSV=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.csv',lcmDataDir,studyVec(sCnt),scanName));
                    fprintf(unit, ' FILPS=''%s''\n', sprintf('%sAnalysis/LCModel_%03i/%s.ps',lcmDataDir,studyVec(sCnt),scanName));
