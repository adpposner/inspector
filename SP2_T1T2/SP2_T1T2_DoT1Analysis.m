%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function  f_succ = SP2_T1T2_DoT1Analysis
%%
...
if flag.t1t2AnaSignFlip
    amplVec(1:t1t2.anaSignFlipN) = -amplVec(1:t1t2.anaSignFlipN);
end

%--- info printout ---
fprintf('Amplitude vector: %s\n',SP2_Vec2PrintStr(amplVec,0));
...
%--- T1 analysis ---
if flag.t1t2AnaMode           % fixed T1 components
    if amplVec(end)>0
    ...
    switch t1t2.anaTConstN
        case 1
            ...
            [t1t2.t1spec(ptCnt,:),t1t2.res2norm(ptCnt)] = lsqcurvefit('SP2_T1T2_FunExp01_T1fix_offset',coeffStart,t1t2.delays,amplVec,lb,ub,options);
            t1t2.satRecSpecFit(ptCnt,:)  = SP2_T1T2_FunExp01_T1fix_offset(t1t2.t1spec(ptCnt,:),t1t2.delays);
            t1t2.satRecSpecFine(ptCnt,:) = SP2_T1T2_FunExp01_T1fix_offset(t1t2.t1spec(ptCnt,:),t1t2.satRecFine);
        case 2
            if flag.t1t2NeighInit && ptCnt>t1t2.anaMinI         % init with neighbor && not 1st fit
                coeffStart = t1t2.t1spec(ptCnt-1,:);
            else                
    ...
    %--- T1 fitting limits ---
    t1min = 1e-2;               % overall minimum T1
    t1max = 5000;               % overall maximum T1
    if flag.t1t2AnaFitOffset            % offset 
        if flag.t1t2AnaFlex1Fix         % flexible with 1st TC fixed
            ...
            %--- T1 fitting procedure ---
            % note that the offset adds one parameter and the one fixed TC
            % requires one less, i.e. overall the 2*t1t2.anaTConstFlexN
            % remains
            coeffStart          = abs(amplVec(1)-amplVec(end))/t1t2.anaTConstFlexN*ones(1,2*t1t2.anaTConstFlexN);    % starting coefficients, amplitudes
            coeffStart(2:2:end) = flipdim(t1max./(2*(1:t1t2.anaTConstFlexN)),2);                 % initial estimate of time constants, small to t2max/2, shortest 1st
            coeffStart(end)     =  amplVec(1);                          % starting offset: last data point
            lb            = [zeros(1,2*t1t2.anaTConstFlexN-1) -1e10];   % lower bound, amplitudes
            lb(2:2:end-1) = t1min;                                      % lower bound, T1's
            ub            = 1e10*ones(1,2*t1t2.anaTConstFlexN);         % upper bound, amplitude
            ub(2:2:end-1) = t1max;                                      % upper bound, T1's       
            eval(['[coeffFit,res2norm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(''SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '_OneFix_Offset'',coeffStart,t1t2.delays,amplVec,lb,ub,options);'])
            ...
            eval(['t1t2.amplFit     = SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '_OneFix_Offset(coeffFit,t1t2.delays);'])
            eval(['t1t2.amplFitFine = SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '_OneFix_Offset(coeffFit,t1t2.delaysFine);'])
            ...
            for tCnt = 1:t1t2.anaTConstFlexN
                t1t2.amplSepFitFine(:,tCnt) = SP2_T1T2_T1FunExp01(coeffFit(2*(tCnt-1)+1:2*tCnt),t1t2.delaysFine);
            end
            t1t2.amplSepFitFine(:,end) = ones(length(t1t2.delaysFine),1)*coeffFit(end);       % offset
            
            %--- amplitude / T1 extraction ---
            t1t2.fitAmpl        = coeffFit(1:2:length(coeffFit));               % keep amplitudes + offset, discard T1's
            ...
            %--- T1 fitting procedure ---
            coeffStart          = [abs(amplVec(1)-amplVec(end))/t1t2.anaTConstFlexN*ones(1,2*t1t2.anaTConstFlexN) amplVec(1)];       % starting coefficients, amplitudes
            coeffStart(2:2:end) = flipdim(t1max./(1:t1t2.anaTConstFlexN),2);                 % initial estimate of time constants, small to t2max/2, shortest 1st
            lb          = [zeros(1,2*t1t2.anaTConstFlexN) -1e10];   % lower bound, amplitudes
            lb(2:2:end) = t1min;                                    % lower bound, T1's
            ub          = 1e10*ones(1,2*t1t2.anaTConstFlexN+1);     % upper bound, amplitude
            ub(2:2:end) = t1max;                                    % upper bound, T1's       
            eval(['[coeffFit,res2norm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(''SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '_Offset'',coeffStart,t1t2.delays,amplVec,lb,ub,options);'])
            ...
            %--- amplitude / T2 fit extraction (for display)
            eval(['t1t2.amplFit     = SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '_Offset(coeffFit,t1t2.delays);'])
            eval(['t1t2.amplFitFine = SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '_Offset(coeffFit,t1t2.delaysFine);'])
            t1t2.amplSepFitFine = zeros(length(t1t2.delaysFine),t1t2.anaTConstFlexN+1);       % init array of individual T2 components
            for tCnt = 1:t1t2.anaTConstFlexN
                t1t2.amplSepFitFine(:,tCnt) = SP2_T1T2_T1FunExp01(coeffFit(2*(tCnt-1)+1:2*tCnt),t1t2.delaysFine);
            end
        ...
        if flag.t1t2AnaFlex1Fix         % flexible with 1st TC fixed
            ...
            %--- T1 fitting procedure ---
            coeffStart          = abs(amplVec(1)-amplVec(end))/t1t2.anaTConstFlexN*ones(1,2*(t1t2.anaTConstFlexN-1)+1);     % starting coefficients, amplitudes
            coeffStart(2:2:end) = flipdim(t1max./(2*(1:(t1t2.anaTConstFlexN-1))),2);                 % initial estimate of time constants, small to t2max/2, shortest 1st
            ...
            ub(2:2:end) = t1max;                                        % upper bound, T1's       
            eval(['[coeffFit,res2norm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(''SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '_OneFix'',coeffStart,t1t2.delays,amplVec,lb,ub,options);'])
            ...
            %--- amplitude / T2 fit extraction (for display)
            eval(['t1t2.amplFit     = SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '(coeffFit,t1t2.delays);'])
            eval(['t1t2.amplFitFine = SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) '(coeffFit,t1t2.delaysFine);'])
            ...
            ub          = 1e10*ones(1,2*t1t2.anaTConstFlexN);       % upper bound, amplitude
            ub(2:2:end) = t1max;                                    % upper bound, T1's       
            eval(['[coeffFit,res2norm,residual,exitflag,output,lambda,jacobian] = lsqcurvefit(''SP2_T1T2_T1FunExp0' num2str(t1t2.anaTConstFlexN) ''',coeffStart,t1t2.delays,amplVec,lb,ub,options);'])
...
fprintf('T1 time constants: %s s\n',SP2_Vec2PrintStr(t1t2.fitTConst,3));
if flag.t1t2AnaFitOffset            % offset 
    fprintf('T1 amplitudes:     %s a.u.\n',SP2_Vec2PrintStr(t1t2.fitAmpl(1:end-1),0));
    fprintf('T1 ampl. ratio:    %s%%\n',SP2_Vec2PrintStr(t1t2.fitAmplPortion(1:end-1)));
...
t2AnaFh = figure;
if flag.t1t2AnaData==1                                              % FID
    set(t2AnaFh,'NumberTitle','off','Name',sprintf(' T1 Analysis of FID Amplitudes (res2norm %.0f)',res2norm),...
        'Position',[314 114 700 500],'Color',[1 1 1]);
elseif flag.t1t2AnaData==2 || flag.t1t2AnaData==3                   % peak heights
    set(t2AnaFh,'NumberTitle','off','Name',sprintf(' T1 Analysis of Peak Heights (res2norm %.0f)',res2norm),...
        'Position',[314 114 700 500],'Color',[1 1 1]);
else                                                                % direct assignment
    set(t2AnaFh,'NumberTitle','off','Name',sprintf(' T1 Analysis of Directly Assigned Vectors (res2norm %.0f)',res2norm),...
        'Position',[314 114 700 500],'Color',[1 1 1]);
end
...
if flag.t1t2AnaFitOffset                    % fit offset included
    ...
    for tCnt = 1:t1t2.anaTConstFlexN
        ...
        if ~flag.t1t2AnaMode && flag.t1t2AnaFlex1Fix && tCnt==t1t2.anaTConstFlexN  
            legendCell{tCnt} = sprintf('T1 %.1f ms (fix), Ampl. %.0f %s %.0f',...
                                       t1t2.fitTConst(tCnt),t1t2.fitAmpl(tCnt),char(177),fitAmplErr(tCnt));
        else    
            legendCell{tCnt} = sprintf('T1 %.1f %s %.1f ms, Ampl. %.0f %s %.0f',...
...
if flag.t1t2AmplShow                        % automated
    for tCnt = 1:t1t2.anaTConstFlexN
        if tCnt==1
    ...
    axis([minXfine maxXfine t1t2.amplShowMin t1t2.amplShowMax])
end 
