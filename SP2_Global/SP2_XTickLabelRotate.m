function hText = SP2_XTickLabelRotate(XTick,rot,varargin)
...
if isempty(get(gca,'XTickLabel')),
    fprintf('xticklabel_rotate : can not process, either xticklabel_rotate has already been run or XTickLabel field has been erased')
    return
end
...
if length(XTick) ~= length(xTickLabels),
    fprintf('xticklabel_rotate : must have same number of elements in "XTick" and "XTickLabel"')
    return
end
