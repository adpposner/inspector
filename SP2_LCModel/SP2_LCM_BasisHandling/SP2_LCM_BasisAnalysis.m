%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_done = SP2_LCM_BasisAnalysis(datSpec,plotLim,varargin)
%%
...
[targetMinI,targetMaxI,targetPpmZoom,targetSpecZoom,f_done] = ...
    SP2_LCM_ExtractPpmRange(lcm.ppmTargetMin,lcm.ppmTargetMax,lcm.basis.ppmCalib,lcm.basis.sw,datSpec);
...
if flag.lcmAnaSNR
    [noiseMinI,noiseMaxI,noisePpmZoom,noiseSpecZoom,f_done] = ...
        SP2_LCM_ExtractPpmRange(lcm.ppmNoiseMin,lcm.ppmNoiseMax,lcm.basis.ppmCalib,lcm.basis.sw,datSpec);
    if ~f_done
...
if flag.lcmOffset      % ppm window
    ...
    [offsetMinI,offsetMaxI,offsetPpmZoom,offsetSpecZoom,f_done] = ...
        SP2_LCM_ExtractPpmRange(lcm.ppmOffsetMin,lcm.ppmOffsetMax,lcm.basis.ppmCalib,lcm.basis.sw,datSpec);
    if ~f_done
...
if flag.lcmAnaFWHM
    %--- FWHM analysis ---
    if flag.lcmAnaSign             % positive sign
        [fwhmPts,minFwhmPts,maxFwhmPts,maxAmpl,maxAmplPts,msg] = SP2_FWHM(targetSpecZoom-specOffset,1,flag.verbose);
    else
        [fwhmPts,minFwhmPts,maxFwhmPts,maxAmpl,maxAmplPts,msg] = SP2_FWHM(-(targetSpecZoom-specOffset),1,flag.verbose);
    end
    if isempty(fwhmPts)
...
if flag.lcmAnaIntegr
    ...
    if flag.lcmAnaSign                 % positive sign
        fprintf('Spectral integration %.2f..%.2f ppm (%.0fpts), offset %.0f, maximum %.0f:\nTotal %.0f, %.0f per point\n',...
        ...
        fprintf('Spectral integration %.2f..%.2f ppm (%.0fpts), offset %.0f, minimum %.0f:\nTotal %.0f, %.0f per point\n',...
