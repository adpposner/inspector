%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_succ = SP2_LCM_AnaDoCalcCRLBTotal( f_plot )
%%
...
fprintf('Spectral noise range %.3f..%.3f ppm, 2nd order corrected:\nmin/max/SD %.1f/%.1f/%.1f\n',noisePpmZoom(1),noisePpmZoom(end),...
        min(noiseSpecZoom),max(noiseSpecZoom),std(noiseSpecZoom))
if flag.lcmSaveLog && ~f_plot
    fprintf(lcm.log,'Spectral noise range %.3f..%.3f ppm, 2nd order corrected:\nmin/max/SD %.1f/%.1f/%.1f\n',noisePpmZoom(1),noisePpmZoom(end),...
            min(noiseSpecZoom),max(noiseSpecZoom),std(noiseSpecZoom));
end
...
%--- noise ---
% noise  = std(noiseSpecZoom)/sqrt(length(noiseSpecZoom));
noise  = std(noiseSpecZoom)/sqrt(lcm.nspecC);
% noise  = std(residSpecZoom)/sqrt(length(anaAllInd));
% noise  = std(noiseSpecZoom)/sqrt(length(anaAllInd));
% noise = std(noiseFidZoom);
...
nLinked = 1;          % number of linked metabolites
...
%     function of the overall target amplitude
    cKStr = 'basisFid(:,metabCnt) * lcmAmpVec(metabCnt)';        % this is the way that works        
...
if f_plot && flag.verbose
    ...
    set(lcm.fhFisher,'NumberTitle','off','Name',sprintf(' Fisher Information Matrix'),...
               'Position',[100 100 692 550],'Color',[1 1 1])
...
if f_plot || flag.verbose
    ...
    for mCnt = 1:nMetab
        fprintf('%s:%s%.3f%%\n',basisNames{mCnt},SP2_NSpaces(mStrMax-length(lcm.anaMetabs{mCnt})+2),lcm.fit.crlbAmp(mCnt))   
    end
    fprintf('\n');
end
% lcm.anaCrlb = crlbAmp;
...
if flag.lcmSaveLog && ~f_plot
    %--- info printout: individual pars ---
    % in original order
    fprintf(lcm.log,'CRLB(amplitude) = %s%%\n',SP2_Vec2PrintStr(lcm.fit.crlbAmp,3));
    if flag.lcmAnaLb && ~flag.lcmLinkLb
        fprintf(lcm.log,'CRLB(LB)        = %s Hz\n',SP2_Vec2PrintStr(lcm.fit.crlbLb,3));
    end
    if flag.lcmAnaGb && ~flag.lcmLinkGb
        fprintf(lcm.log,'CRLB(GB)        = %s Hz\n',SP2_Vec2PrintStr(lcm.fit.crlbGb,3));
    end
    if flag.lcmAnaShift && ~flag.lcmLinkShift
...
if f_plot
    ...
    set(lcm.fhCorrAmp,'NumberTitle','off','Name',sprintf(' LCModel Correlation Matrix: Amplitude'),...
               'Position',[500 320 692 550],'Color',[1 1 1])
...
if f_plot && flag.verbose
    ...
    if exist('corrMatLb','var')
        ...
        set(lcm.fhCorrLb,'NumberTitle','off','Name',sprintf(' LCModel Correlation Matrix: Lorentzian Linebroadening'),...
                   'Position',[510 330 692 550],'Color',[1 1 1])
    ...
    if exist('corrMatGb','var')
        ...
        set(lcm.fhCorrGb,'NumberTitle','off','Name',sprintf(' LCModel Correlation Matrix: Gaussian Linebroadening'),...
                   'Position',[520 340 692 550],'Color',[1 1 1])
    ...
    if exist('corrMatShift','var')
        ...
        set(lcm.fhCorrShift,'NumberTitle','off','Name',sprintf(' LCModel Correlation Matrix: Frequency Shift'),...
                   'Position',[530 350 692 550],'Color',[1 1 1])
