%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
    function f_succ = SP2_LCM_AnaDoMonteCarloSim( f_mcIdent )
%% 
...
global loggingfile lcm flag syn

FCTNAME = 'SP2_LCM_AnaDoMonteCarloSim';
...
flagSynNoiseKeep  = flag.synNoiseKeep;
flag.synNoiseKeep = 0;          % new calculation
...
for nCnt = lcm.mc.nPrev+1:lcm.mc.nPrev+lcm.mc.n
    ...
    if flag.lcmMCarloInit && lcm.mc.initSpread>0             % init with somewhat randomized previous result
        ...
        if f_lb
            lcm.anaLb(lcm.fit.applied) = lcm.anaLb(lcm.fit.applied) + ...
                                         max(abs(lcm.anaLb(lcm.fit.applied)), 5*ones(1,lcm.fit.appliedN)) .* ...
                                         lcm.mc.initSpread/100.*randn(1,lcm.fit.appliedN);
        end
        %--- GB ---
        if f_gb
            lcm.anaGb(lcm.fit.applied) = lcm.anaGb(lcm.fit.applied) + ...
                                         max(abs(lcm.anaGb(lcm.fit.applied)), 5*ones(1,lcm.fit.appliedN)) .* ...
                                         lcm.mc.initSpread/100.*randn(1,lcm.fit.appliedN);
        end
        %--- shift ---
        if f_shift 
        ...
        %--- PHC0 ---
        if f_phc0 
            lcm.anaPhc0 = lcm.anaPhc0 + max(abs(lcm.anaPhc0),10)*lcm.mc.initSpread/100*randn(1,1);
        end
    ...
    if f_mcIdent                    % synthesized grid of singlets
        if ~SP2_Syn_DoSimSinglets(1)            % 1: use current singlet grid
            fprintf('%s ->\nSimulation of singlet spectrum failed. Program aborted.\n\n',FCTNAME);
            return
        end
    else                            % synthesized brain MRS
...
%             lcm.mc.concSdTrace(nCnt,lcm.fit.appliedN+1:end) = 100*std(lcm.mc.concMat(1:nCnt,lcm.fit.appliedN+1:end))./abs(mean(lcm.mc.concMat(1:nCnt,lcm.fit.appliedN+1:end)));    % [%]
    ...
    %--- save analysis and noise figures of 1st run to file ---
    if nCnt==1
        %-------------------------------------------
        ...
        if isfield(lcm,'fhLcmFitSpecSummary')
            if ishandle(lcm.fhLcmFitSpecSummary)
                if f_mcIdent                    % synthesized grid of singlets
                    lcmSummaryFigPath = [resultDir 'SPX_LcmMCarlo_SimSinglets1stSummary.fig'];
                else                            % synthesized brain MRS
                    lcmSummaryFigPath = [resultDir 'SPX_LcmMCarlo_SimBrains1stSummary.fig'];
                end
        ...
        if isfield(lcm,'fhLcmSpec')
            if ishandle(lcm.fhLcmSpec)
                if f_mcIdent                    % synthesized grid of singlets
                    lcmNoiseFigPath = [resultDir 'SPX_LcmMCarlo_SimSinglets1stNoise.fig'];
                else                            % synthesized brain MRS
                    lcmNoiseFigPath = [resultDir 'SPX_LcmMCarlo_SimBrain1stNoise.fig'];
                end
...
if f_phc0
    ...
    fprintf('Ratio: Mean CRLB / PHC0 SD:\n%s, mean %.2f\n\n',SP2_Vec2PrintStr(phc0RatioCrlbSd,3),mean(phc0RatioCrlbSd));
end

%--- noise statistics ---
fprintf('MC reference noise %.4f\n',syn.noiseAmp);
...
fprintf(unit,'SD:        %s%%\n',SP2_Vec2PrintStr(100*std(lcm.mc.concMat)./mean(lcm.mc.concMat),4));
fprintf(unit,'CRLB Mean: %s%%\n',SP2_Vec2PrintStr(meanConcCrlbMat,4));
fprintf(unit,'CRLB SD:   %s%%\n',SP2_Vec2PrintStr(stdConcCrlbMat,4));
...
fprintf(unit,'MC reference noise %.4f\n',syn.noiseAmp);
...
legend('CRLB: Mean','CRLB: Mean-SD','CRLB: Mean+SD','CRLB: Indiv.','Conc: SD','Location','Best')
...
flag.synNoiseKeep = flagSynNoiseKeep;
