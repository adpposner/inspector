...
global loggingfile stab flag data
...
%--- data manipulation ---
% scaling of spectral range
f_scale     = 1;                % up/down-scaling of spectral range
scaleMin    = 550;              % minimum point of scaling region
scaleMax    = 800;              % maximum point of scaling region
scaleFac    = 2;                % scaling factor
% sine modulation of spectral range
f_sineMod   = 1;                % sine modulation of spectral range
modMin      = 1350;             % minimum point of modulation region
modMax      = 1600;             % maximum point of modulation region
modFac      = 1.5e6;            % scaling factor
...
for bCnt = 1:stab.ppmBins
    minPpm(1,bCnt) = -stab.sw/2 + stab.ppmCalib + (bCnt-1)*stab.sw/stab.ppmBins;    % lower ppm limit of considered noise region
    maxPpm(1,bCnt) = minPpm(1,bCnt) + stab.sw/stab.ppmBins;                       % upper ppm limit of considered noise region
    % determination of correspondind indices
    [minInd(1,bCnt),maxInd(1,bCnt),ppmVec,specVec] = SP2_ExtractPpmRange(minPpm(1,bCnt),maxPpm(1,bCnt),stab.ppmCalib,stab.sw,stab.spec(1,:));
end
...
totalMean = mean(stab.spec(stab.specFirst:stab.specLast,:),2)';
totalMax  = max(stab.spec(stab.specFirst:stab.specLast,:),[],2)';
totalStd  = std(stab.spec(stab.specFirst:stab.specLast,:),0,2)';

%--- further analysis of total, i.e. spectrum statistics  ---
fprintf('Whole spectrum:\n');
fprintf('Series mean:     std.dev. %.0f, mean %.0f\n',std(totalMean),mean(totalMean));
fprintf('Series max:      std.dev. %.0f, mean %.0f\n',std(totalMax),mean(totalMax));
fprintf('Series std.dev.: std.dev. %.0f, mean %.0f\n',std(totalStd),mean(totalStd));


%--- frequency range dependence of noise ---
for bCnt = 1:stab.ppmBins
    binSpec = stab.spec(stab.specFirst:stab.specLast,minInd(1,bCnt):maxInd(1,bCnt));
    binMean(bCnt,:)    = mean(binSpec,2);           % average value of noise region
    binMax(bCnt,:)     = max(binSpec,[],2);         % maximum value of noise region
    binStd(bCnt,:)     = std(binSpec,0,2);          % standard deviation of noise
...
if flag.stabTotSel
    ...
    maxLenStdDev = 0;
    for bCnt = 1:stab.ppmBins
        if length(sprintf('%.1f',minPpm(1,bCnt)))>maxLenMinPpm;    
            maxLenMinPpm = length(sprintf('%.1f',minPpm(1,bCnt)));
        end
        if length(sprintf('%.1f',maxPpm(1,bCnt)))>maxLenMaxPpm;    
            maxLenMaxPpm = length(sprintf('%.1f',maxPpm(1,bCnt)));
        end
    ...
    for bCnt = 1:stab.ppmBins
        ...
        numLen = length(sprintf('%.1f',minPpm(1,bCnt)));
        for jcnt = 1:maxLenMinPpm-numLen
            printStr = [printStr ' '];
        end
        printStr = [printStr '%.1f to '];
        numLen = length(sprintf('%.1f',maxPpm(1,bCnt)));
